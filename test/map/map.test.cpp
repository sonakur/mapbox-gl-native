...
TEST(Map, RendererState) {
    ...
    test.map.getStyle().loadJSON(*mapbox::base::readFile("test/fixtures/api/empty.json"));
    ...
    test.map.getStyle().addImage(std::make_unique<style::Image>("default_marker", decodeImage(*mapbox::base::readFile("test/fixtures/sprites/default_marker.png")), 1.0));
...
TEST(Map, Offline) {
    ...
    auto expiredItem = [] (const std::string& path) {
        Response response;
        response.data = std::make_shared<std::string>(*mapbox::base::readFile("test/fixtures/map/offline/"s + path));
...
TEST(Map, SetStyleDefaultCamera) {
    ...
    test.map.getStyle().loadJSON(*mapbox::base::readFile("test/fixtures/api/empty-zoomed.json"));
...
TEST(Map, StyleFresh) {
    ...
    Response response;
    response.data = std::make_shared<std::string>(*mapbox::base::readFile("test/fixtures/api/empty.json"));
...
TEST(Map, StyleEarlyMutation) {
    ...
    Response response;
    response.data = std::make_shared<std::string>(*mapbox::base::readFile("test/fixtures/api/water.json"));
    test.fileSource->respond(Resource::Style, response);
...
TEST(Map, WithoutVAOExtension) {
    ...
    test.map.getStyle().loadJSON(*mapbox::base::readFile("test/fixtures/api/water.json"));
    ...
    test.fileSource->response = [] (const Resource& res) -> optional<Response> {
        if (res.url == "asset://tile.png") {
            ...
            response.data = std::make_shared<std::string>(
                *mapbox::base::readFile("test/fixtures/map/disabled_layers/tile.png"));
...
TEST(Map, TEST_DISABLED_ON_CI(ContinuousRendering)) {
    ...
    map.getStyle().loadJSON(*mapbox::base::readFile("test/fixtures/api/water.json"));
...
TEST(Map, Issue12432) {
    ...
    test.fileSource->tileResponse = [&](const Resource&) {
        Response result;
        result.data = std::make_shared<std::string>(*mapbox::base::readFile("test/fixtures/map/issue12432/0-0-0.mvt"));
        return result;
    };
