...
TEST(Map, PrefetchTiles) {
    ...
    fileSource->response = [&] (const Resource& res) -> optional<Response> {
        static std::string tile = *mapbox::base::readFile("test/fixtures/map/prefetch/tile.png");
    ...
    auto checkTilesForZoom = [&](int zoom, const std::vector<int>& expected) {
        ...
        map.getStyle().loadJSON(*mapbox::base::readFile("test/fixtures/map/prefetch/empty.json"));
        map.getStyle().loadJSON(*mapbox::base::readFile("test/fixtures/map/prefetch/style.json"));
