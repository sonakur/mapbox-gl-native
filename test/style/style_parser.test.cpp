...
TEST_P(StyleParserTest, ParseStyle) {
    ...
    infoDoc.Parse<0>(mapbox::base::readFile(base + ".info.json")->c_str());
    ...
    style::Parser parser;
    auto error = parser.parse(*mapbox::base::readFile(base + ".style.json"));
...
TEST(StyleParser, FontStacks) {
    style::Parser parser;
    parser.parse(*mapbox::base::readFile("test/fixtures/style_parser/font_stacks.json"));
