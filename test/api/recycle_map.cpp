...
TEST(API, RecycleMapUpdateImages) {
    ...
    auto loadStyle = [&](auto markerName, auto markerPath) {
        ...
        map->getStyle().loadJSON(*mapbox::base::readFile("test/fixtures/api/empty.json"));
        map->getStyle().addSource(std::move(source));
        map->getStyle().addLayer(std::move(layer));
        map->getStyle().addImage(std::make_unique<style::Image>(markerName, decodeImage(*mapbox::base::readFile(markerPath)), 1.0));
    };
