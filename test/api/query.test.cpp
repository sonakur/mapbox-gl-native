...
public:
    QueryTest() {
        map.getStyle().loadJSON(*mapbox::base::readFile("test/fixtures/api/query_style.json"));
        map.getStyle().addImage(std::make_unique<style::Image>("test-icon",
            decodeImage(*mapbox::base::readFile("test/fixtures/sprites/default_marker.png")), 1.0));
...
std::vector<Feature> getTopClusterFeature(QueryTest& test) {
    test.fileSource->sourceResponse = [&] (const Resource& resource) {
        ...
        Response response;
        response.data = std::make_unique<std::string>(*mapbox::base::readFile("test/fixtures/supercluster/places.json"s));
        return response;
    };
