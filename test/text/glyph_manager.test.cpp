...
TEST(GlyphManager, LoadingSuccess) {
    ...
    test.fileSource.glyphsResponse = [&] (const Resource& resource) {
        ...
        Response response;
        response.data = std::make_shared<std::string>(*mapbox::base::readFile("test/fixtures/resources/glyphs.pbf"));
        return response;
    };
...
TEST(GlyphManager, LoadLocalCJKGlyphAfterLoadingRangeFromURL) {
    ...
    test.fileSource.glyphsResponse = [&] (const Resource&) {
        ...
        Response response;
        response.data = std::make_shared<std::string>(*mapbox::base::readFile("test/fixtures/resources/glyphs-12244-12543.pbf"));
        return response;
...
TEST(GlyphManager, LoadingInvalid) {
    ...
    test.fileSource.glyphsResponse = [&] (const Resource& resource) {
        EXPECT_EQ(Resource::Kind::Glyphs, resource.kind);
        Response response;
        response.data = std::make_shared<std::string>(*mapbox::base::readFile("test/fixtures/resources/fake_glyphs-0-255.pbf"));
        return response;
    };
