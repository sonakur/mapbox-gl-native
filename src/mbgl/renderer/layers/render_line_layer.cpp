...
bool RenderLineLayer::queryIntersectsFeature(
        ...
        const mat4&,
        const PropertyMap& featureState) const {
...
float RenderLineLayer::getLineWidth(const GeometryTileFeature& feature, const float zoom, const PropertyMap& featureState) const {
    const auto& evaluated = static_cast<const LineLayerProperties&>(*evaluatedProperties).evaluated;
