...
inline optional<std::string> featureIDtoString(const FeatureIdentifier& id) {
    if (id.is<NullValue>()) return optional<std::string>();

    optional<std::string> idStr = id.match([](std::string value_) { return value_; },
...
public:
    ...
    virtual void updateVertexVector(std::size_t, std::size_t, const GeometryTileFeature&, const PropertyMap&) = 0;
...
public:
    ...
    void updateVertexVector(std::size_t, std::size_t, const GeometryTileFeature&, const PropertyMap&) override {}
...
public:
    ...
    void updateVertexVector(std::size_t start, std::size_t end, const GeometryTileFeature& feature, const PropertyMap& state) override {
        using style::expression::EvaluationContext;

        auto evaluated = expression.evaluate(EvaluationContext(0, &feature, &state), defaultValue);
        this->statistics.add(evaluated);
        auto value = attributeValue(evaluated);
