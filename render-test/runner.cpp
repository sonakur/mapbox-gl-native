...
bool TestRunner::checkResults(mbgl::PremultipliedImage&& actualImage, TestMetadata& metadata) {
    ...
    if (getenv("UPDATE_PLATFORM")) {
        ...
        mapbox::base::writeFile(expectations.back().string() + "/expected.png", mbgl::encodePNG(actualImage));
        return true;
    } else if (getenv("UPDATE_DEFAULT")) {
        mapbox::base::writeFile(base + "/expected.png", mbgl::encodePNG(actualImage));
        return true;
    } else if (getenv("UPDATE_METRICS")) {
        if (!metadata.metrics.isEmpty()) {
            mbgl::filesystem::create_directories(expectations.back());
            mapbox::base::writeFile(expectations.back().string() + "/metrics.json", serializeMetrics(metadata.metrics));
            return true;
        }
    }

    mapbox::base::writeFile(base + "/actual.png", metadata.actual);
#endif
    ...
    for (const auto& entry: expectedImagesPaths) {
        auto maybeExpectedImage = mapbox::base::readFile(entry);
...
#if !TEST_READ_ONLY
        mapbox::base::writeFile(base + "/diff.png", metadata.diff);
#endif
...
bool TestRunner::runOperations(const std::string& key, TestMetadata& metadata) {
    ...
    // addImage | updateImage
        ...
        auto maybeImage = mapbox::base::readFile(filePath);
        if (!maybeImage) {
